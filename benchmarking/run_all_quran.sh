...
program_to_execute="benchmarking/all_quran.py" # Replace with the actual program name or path
...
#     python "$program_to_execute" --in_file "$file" --out_file "benchmarking/output_all_quran/""$filename"
...
for file in "$json_dir"/*; do
  ...
  if [[ -f "$file" && "$file" == *.json ]]; then
    ...
    echo "Processing file: $filename"
    # Execute the program with the current JSON file as an argument
    python "benchmarking/evaluate.py" --in_file "benchmarking/output_all_quran/""$filename"
    # You can add more commands here if needed for each file
  fi
done
